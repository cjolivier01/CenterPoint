...
def main():
    cfg = Config.fromfile('configs/nusc/pp/hm_nusc_centerpoint_pp_02voxel_two_pfn_10sweep_demo.py')
    ...
    checkpoint = load_checkpoint(model, 'pretrained/pillar/latest.pth', map_location="cpu")
    model.eval()
    ...
    for i, data_batch in enumerate(data_loader):
        ...
        um_points = data_batch['points']
        if isinstance(um_points, list):
            um_points = torch.stack(um_points)
        points = um_points[:, 1:4].cpu().numpy()
        with torch.no_grad():
